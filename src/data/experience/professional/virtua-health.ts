import type { ExpItem } from '../../types'

const virtuaHealth: ExpItem = {
  company: 'Virtua Health',
  role: 'IT Intern',
  period: 'May 2025 – Sept 2025',
  location: 'Marlton, NJ',
  type: 'Internship',
  bullets: [
    'Optimized the ID database by vacating 9,000 invalid cells (10% of the total database) to resolve an ID creation bottleneck.',
    'Analyzed over 1,000 client tickets to identify key pain points in the Oracle ERP system, informing future IT reforms.',
  ],
  gallery: [
    '/assets/gallery/virtua/virtua.webp',
  ],
}

export default virtuaHealth
